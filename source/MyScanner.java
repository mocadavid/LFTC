...
public class MyScanner {
    ...
    public MyScanner(SymbolTableWithHash<String, String> symbolTable) {
        ...
        fa.loadFile("src/ro/ubb/FA/identifier");
        fa2.loadFile("src/ro/ubb/FA/integer-constant");
    }
    ...
    private boolean isConstant(String token, String lastSign){
        if (token.matches("'[a-zA-Z0-9]{1}'"))
            ...
            if (lastSign!=null){
                return fa2.isAccepted(lastSign+token);
            }
            else{
                return fa2.isAccepted(token);
            }
